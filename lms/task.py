...
def send_course_update_notifications(course_id):
    ...
    try:
        ...
        # Находим подписаных пользователей
        subscriptions = Subscription.objects.filter(course=course)

        # Если у курса нет подписчиков
        if not subscriptions.exists():
            print(f"У курса {course.title} нет подпичиков")
...
@shared_task
def send_update_email_to_user(user_email, user_name, course_title, course_id):
    """ Отпраялет одно письмо одному пользователю """

    try:
        # Создаём HTML письмо
        html_message = render_to_string('emails/course_update_email.html', {
